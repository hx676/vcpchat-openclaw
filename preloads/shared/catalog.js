...
function createCatalog(ops) {
    return {
        ...
        creditListCases: query(() => ops.invoke('credit:list-cases')),
        creditCreateCase: query((payload) => ops.invoke('credit:create-case', payload)),
        creditGetCase: query((caseId) => ops.invoke('credit:get-case', caseId)),
        creditUpdateCase: query((caseId, patch) => ops.invoke('credit:update-case', caseId, patch)),
        creditSelectMaterials: query((caseId) => ops.invoke('credit:select-materials', caseId)),
        creditExtractMaterial: query((caseId, materialId) => ops.invoke('credit:extract-material', caseId, materialId)),
        creditUpdateMaterialType: query((caseId, materialId, materialType) => ops.invoke('credit:update-material-type', caseId, materialId, materialType)),
        creditSaveExtract: query((caseId, materialId, patch) => ops.invoke('credit:save-extract', caseId, materialId, patch)),
        creditSaveExternalData: query((caseId, data) => ops.invoke('credit:save-external-data', caseId, data)),
        creditUpdateFieldEvidence: query((caseId, fieldName, patch) => ops.invoke('credit:update-field-evidence', caseId, fieldName, patch)),
        creditGetAssistantChat: query((caseId) => ops.invoke('credit:get-assistant-chat', caseId)),
        creditAppendAssistantChat: query((caseId, messages) => ops.invoke('credit:append-assistant-chat', caseId, messages)),
        creditSendAssistantMessage: query((caseId, message) => ops.invoke('credit:assistant-message', caseId, message)),
        creditImportExternalJson: query((caseId) => ops.invoke('credit:import-external-json', caseId)),
        creditRunRules: query((caseId) => ops.invoke('credit:run-rules', caseId)),
        creditGenerateReport: query((caseId) => ops.invoke('credit:generate-report', caseId)),
        creditSaveReport: query((caseId, report) => ops.invoke('credit:save-report', caseId, report)),
        creditExportReport: query((caseId, reportId, format) => ops.invoke('credit:export-report', caseId, reportId, format)),
        creditOpenDataFolder: query(() => ops.invoke('credit:open-data-folder')),
        gatewayGetStatus: query(() => ops.invoke('gateway:get-status')),
        gatewayStartCcproxy: query(() => ops.invoke('gateway:start-ccproxy')),
        gatewayRestartCcproxy: query(() => ops.invoke('gateway:restart-ccproxy')),
        gatewayStopCcproxy: query(() => ops.invoke('gateway:stop-ccproxy')),
        gatewaySaveActiveAccount: query((label) => ops.invoke('gateway:save-active-account', label)),
        gatewaySwitchAccount: query((fileName, restart = true) => ops.invoke('gateway:switch-account', fileName, restart)),
        gatewaySwitchLatestAccount: query((restart = true) => ops.invoke('gateway:switch-latest-account', restart)),
        gatewayDeleteAccount: query((fileName) => ops.invoke('gateway:delete-account', fileName)),
        gatewayLaunchLogin: query((label) => ops.invoke('gateway:launch-login', label)),
        gatewayOpen: query((target) => ops.invoke('gateway:open', target)),
        gatewayTestVcp: query(() => ops.invoke('gateway:test-vcp')),
        gatewayTestAccount: query((fileName) => ops.invoke('gateway:test-account', fileName)),
        gatewayTestAllAccounts: query(() => ops.invoke('gateway:test-all-accounts')),
